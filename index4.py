...
class Window(tk.Tk):
    def __init__(self, **kwargs):
        ...
        ttkStyle.configure('gridEntry.TEntry', font=(
            'Helvetica', '16'), foreground='#333333')
        ...
        # 計算鍵
        commitBtn = ttk.Button(commitFrame, text="計算BMI",
                               command=self.check_value)
        commitBtn.grid(column=0, row=0)
        # 清除鍵
        clearBtn = ttk.Button(commitFrame, text="清除欄位",
    ...
    def check_value(self):
        ...
        birthday = datetime.date(int(birth_get[0:4]), int(
            birth_get[5:7]), int(birth_get[8:]))
