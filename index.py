...
class Window(tk.Tk):
    def __init__(self, **kwargs):
        ...
        heighEntry = ttk.Entry(bottomFrame, style='gridEntry.TEntry')
        heighEntry.grid(column=1, row=3, sticky=tk.W, padx=10)

        ttk.Label(bottomFrame, text='體重(kg):', style='gridLabel.TLabel').grid(
            column=0, row=4, sticky=tk.E)
        weighEntry = ttk.Entry(bottomFrame, style='gridEntry.TEntry')
        weighEntry.grid(column=1, row=4, sticky=tk.W, padx=10)

        messageText = tk.Text(bottomFrame, height=5, width=20,state=tk.DISABLED)
        messageText.grid(column=0, row=5, sticky=tk.W+tk.E+tk.N+tk.S, columnspan=2,padx=10)
